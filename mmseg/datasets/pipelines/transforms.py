            ...
            if(results[key].shape[0] != 2):
                if self.keep_ratio:
                    ...
                    gt_seg = mmcv.imresize(
                        results[key], results['scale'], interpolation='nearest')
            elif(results[key].shape[0] == 2):
                if self.keep_ratio:
                    gt_seg1 = mmcv.imrescale(
                        results[key][0], results['scale'], interpolation='nearest')
                    gt_seg2 = mmcv.imrescale(
                        results[key][1], results['scale'], interpolation='nearest')
                else:
                    gt_seg1 = mmcv.imresize(
                        results[key][0], results['scale'], interpolation='nearest')
                    gt_seg2 = mmcv.imresize(
                        results[key][1], results['scale'], interpolation='nearest')
                gt_seg = np.zeros((2,gt_seg1.shape[0],gt_seg1.shape[1]))
                gt_seg[0] = gt_seg1
                gt_seg[1] = gt_seg2
            results[key] = gt_seg
        ...
        for key in results.get('seg_fields', []):
            ...
            #改，添加了shape的判断
                ...
                src2 = mmcv.impad(
                    ...
                    shape=results['pad_shape'][:2],
                    pad_val=self.seg_pad_val)
                results[key] = np.zeros((2,src1.shape[0],src1.shape[1]))
                results[key][0] = src1
                results[key][1] = src2
            #pdb.set_trace()
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            ...
            if(results[key].shape[0] != 2):
                ...
                src1 = results[key][0]
                src2 = results[key][1]
                src1 = self.crop(src1, crop_bbox)
                src2 = self.crop(src2, crop_bbox)
                results[key] = np.zeros((2,src1.shape[0],src1.shape[1]))
                #results[key] = float()
                #pdb.set_trace()
                results[key][0] = src1
